...
def select_pairs(search_results: dict[tuple[int,int], list[ASFProduct]],
    ...
    if not isinstance(search_results, dict):
        raise ValueError(f'search_results need to be a dict from asf_search, got {type(search_results)}')
    
    pairs_group = defaultdict(list)
    for key, search_result in search_results.items():
        if not isinstance(search_results, dict):
            raise ValueError(f'search_results need to be a dict of list of ASFProduct from asf_search, got {type(search_results)} for key {key}')
